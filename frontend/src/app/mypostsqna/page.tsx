...
const MyPostsQnA = () => {
    ...
    useEffect(() => {
      ...
      const getAllPosts = async (user_id: number | null | undefined, searchTerm?: string) => {
        try{
            setLoading(true)
            let url = `http://localhost:8000/postsqna/user/${user_id}`;
            if(searchTerm){
                url = `http://localhost:8000/postsqna/user/${user_id}/${searchTerm}`
            }
    ...
    const handleEditedPostSubmit = async () => {
        ...
        try {
            const response = await axios.put("http://localhost:8000/postsqna/", { post: newPostContent, title: newPostTitle, id: selectedPost.id });
    ...
    const handleDeletePost = async (post_id: number) => {
        try {
            const response = await axios.delete(`http://localhost:8000/postsqna/${post_id}`);
            console.log("Response", response.data);
